            ...
            <div class="rounded-2xl border border-amber-100 bg-amber-50 px-6 py-5 shadow-sm">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-amber-700">You need to complete your profile before applying to jobs. <a
                                href="{{ route('candidate.wizard.show') }}" class="font-semibold underline">Start here</a></p>
                    </div>
                </div>
            </div>
        @endif
            ...
            <div class="px-6 py-6">
                <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between mb-4">
                    ...
                    <a href="{{ route('candidate.applications.index') }}"
                        class="text-sm font-semibold text-blue-600 hover:text-blue-700">View All</a>
                </div>
                                                ...
                                                <tr class="hover:bg-slate-50/50">
                                                    <td class="px-6 py-4 font-semibold text-blue-700">
                                                        <a href="{{ route('candidate.applications.show', $application) }}"
                                                            class="hover:underline">{{ $application->job->title }}</a>
                                                    </td>
                ...
                @else
                    <p class="text-sm text-slate-500">No applications yet. <a href="{{ route('candidate.jobs.index') }}"
                            class="text-blue-600 hover:text-blue-700 font-semibold">Browse jobs</a></p>
                @endif
            </div>
        </div>
                                ...
                                @foreach($upcomingAppointments as $appointment)
                                    <tr class="hover:bg-slate-50/50">
                                        ...
                                        <td class="px-6 py-4 text-slate-800">
                                            {{ $appointment->scheduled_at->format('M d, Y h:i A') }}</td>
                                        <td class="px-6 py-4">
                                            @if($appointment->meeting_link)
                                                <a href="{{ $appointment->meeting_link }}" target="_blank"
                                                    class="text-blue-600 hover:text-blue-700 font-semibold underline">Join Meeting</a>
                                            @else
                                        ...
                                        <td class="px-6 py-4">
                                            <span
                                                class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100">
