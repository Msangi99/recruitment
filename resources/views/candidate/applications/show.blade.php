    ...
    <div class="min-h-screen bg-gray-50">
        ...
        <div class="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
            <div class="px-4 py-6 sm:px-0">
                <div class="mb-6">
                    <a href="{{ route('candidate.applications.index') }}" class="text-indigo-600 hover:text-indigo-900">←
                    ...
                    <div class="px-4 py-5 sm:px-6">
                        <div class="flex justify-between items-start">
                            <div>
                                ...
                                <p class="mt-1 max-w-2xl text-sm text-gray-500">{{ $application->job->company_name }} •
                        ...
                        <dl>
                            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    {!! $application->job->description !!}</dd>
                            </div>
                            ...
                            <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    {{ $application->created_at->format('M d, Y h:i A') }}</dd>
                            </div>
                            @if($application->reviewed_at)
                                <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    <dt class="text-sm font-medium text-gray-500">Reviewed Date</dt>
                                    <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                        {{ $application->reviewed_at->format('M d, Y h:i A') }}</dd>
                                </div>
                            @endif
                            @if($application->status == 'pending')
                                <div class="bg-blue-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                    ...
                                    <dd class="mt-1 text-sm text-blue-700 sm:mt-0 sm:col-span-2">
                                        <strong>Pending:</strong> Your application has been submitted and is waiting for the
