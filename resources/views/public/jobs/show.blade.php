        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16">
                ...
                <div class="hidden md:flex flex-1 justify-center items-center space-x-8">
                    ...
                    <a href="{{ route('public.jobs.index') }}" class="text-blue-600 hover:text-blue-800 font-bold">Find Job</a>
                    <a href="{{ route('public.candidates.index') }}" class="text-blue-600 hover:text-blue-800 font-bold">Find Candidate</a>
                    <a href="{{ route('candidate.consultations.create') }}" class="text-blue-600 hover:text-blue-800 font-bold">Book Appointment</a>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        @if($job->education_level)
                            <div class="bg-gray-50 rounded-lg p-4">
                                ...
                                <p class="text-lg font-semibold text-gray-900">{{ ucfirst(str_replace('-', ' ', $job->education_level)) }}</p>
                            </div>
                        @endif
                    ...
                    <div class="border-t border-gray-200 pt-6">
                        @if($isCandidate)
                            @if($hasApplied)
                                <div class="bg-green-50 border border-green-200 rounded-lg p-4 text-center">
                                    ...
                                    <a href="{{ route('candidate.applications.index') }}" class="text-green-600 hover:text-green-800 text-sm">View your applications →</a>
                                </div>
                            @else
                                <form method="POST" action="{{ route('candidate.jobs.apply', $job) }}" enctype="multipart/form-data" class="space-y-4">
                                    @csrf
                                    
                                    <div>
                                        <label for="cover_letter" class="block text-sm font-medium text-gray-700 mb-2">Cover Letter (Optional)</label>
                                        <textarea name="cover_letter" id="cover_letter" rows="4" class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" placeholder="Tell the employer why you're a great fit for this role...">{{ old('cover_letter') }}</textarea>
                                    </div>

                                    @if($job->requires_video)
                                        <div>
                                            <label for="application_video" class="block text-sm font-medium text-gray-700 mb-2">Video Introduction (Required)</label>
                                            <input type="file" name="application_video" id="application_video" accept="video/mp4,video/mov,video/avi,video/wmv" class="w-full" required>
                                            <p class="mt-1 text-sm text-gray-500">Max 100MB. Accepted formats: MP4, MOV, AVI, WMV</p>
                                        </div>
                                    @endif
                        ...
                        @else
                            <div class="bg-blue-50 border border-blue-200 rounded-lg p-6 text-center">
                                ...
                                <p class="text-gray-600 mb-4">Login or register to apply for this job opportunity.</p>
                                <div class="flex flex-col sm:flex-row justify-center gap-4">
                                    <a href="{{ route('login') }}?redirect={{ urlencode(request()->url()) }}" class="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700">
                                        Login to Apply
                                    </a>
                                    <a href="{{ route('register') }}" class="px-6 py-3 border border-blue-600 text-blue-600 font-semibold rounded-lg hover:bg-blue-50">
