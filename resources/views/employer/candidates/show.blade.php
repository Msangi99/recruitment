                        ...
                        <div class="flex items-center space-x-4">
                            <div class="flex-shrink-0">
                                @if($candidate->candidateProfile && $candidate->candidateProfile->profile_picture)
                                    <img src="{{ asset($candidate->candidateProfile->profile_picture) }}" alt="{{ $candidate->name }}" class="h-20 w-20 rounded-full object-cover border-4 border-white shadow-lg">
                                @else
                                    <div class="h-20 w-20 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center border-4 border-white shadow-lg">
                                        <span class="text-white font-bold text-2xl">{{ substr($candidate->name, 0, 1) }}</span>
                                    </div>
                                @endif
                            </div>
                            <div>
                                ...
                                @if($candidate->candidateProfile && $candidate->candidateProfile->is_available)
                                    <span class="mt-2 inline-block px-3 py-1 text-sm font-medium text-green-700 bg-green-100 rounded-full">Available for Work</span>
                                @else
                                    <span class="mt-2 inline-block px-3 py-1 text-sm font-medium text-gray-700 bg-gray-100 rounded-full">Not Currently Available</span>
                                @endif
                            </div>
                        </div>
                        <div>
                            <a href="{{ route('employer.interviews.create', $candidate) }}" class="px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 shadow-md hover:shadow-lg transition-all duration-200 font-medium">
                                Request Interview
                            </a>
                        </div>
                    </div>
                    ...
                    <dl class="divide-y divide-gray-200">
                        ...
                        @if($candidate->candidateProfile && $candidate->candidateProfile->gender)
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ ucfirst($candidate->candidateProfile->gender) }}</dd>
                            </div>
                        @endif
                        ...
                        @if($candidate->candidateProfile && $candidate->candidateProfile->marital_status)
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ ucfirst($candidate->candidateProfile->marital_status) }}</dd>
                            </div>
                        @endif
                        ...
                        @if($candidate->candidateProfile && $candidate->candidateProfile->citizenship)
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">{{ $candidate->candidateProfile->citizenship }}</dd>
                            </div>
                        @endif
                        ...
                        <div class="bg-gray-50 px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                            ...
                            <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                <span class="text-lg font-semibold text-indigo-600">{{ $candidate->candidateProfile->years_of_experience ?? '0' }}</span> years
                            </dd>
                        </div>
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($candidate->candidateProfile->skills as $skill)
                                            <span class="inline-block bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg px-3 py-2 text-sm text-blue-700 font-medium">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    <div class="flex flex-wrap gap-2">
                                        @foreach($candidate->candidateProfile->languages as $language)
                                            <span class="inline-flex items-center px-3 py-2 rounded-lg text-sm font-medium bg-green-100 text-green-700 border border-green-200">
                                                <svg class="mr-1 h-4 w-4" fill="currentColor" viewBox="0 0 20 20">
                            ...
                            <div class="bg-white px-4 py-5 sm:grid sm:grid-cols-3 sm:gap-4 sm:px-6">
                                ...
                                <dd class="mt-1 text-sm text-gray-900 sm:mt-0 sm:col-span-2">
                                    <span class="text-lg font-semibold text-green-600">
                                        {{ number_format($candidate->candidateProfile->expected_salary, 0) }} {{ $candidate->candidateProfile->currency ?? 'TZS' }}
                                    </span>
                        ...
                        <!-- Privacy Notice -->
                        <div class="bg-yellow-50 px-4 py-5 sm:px-6 border-t-4 border-yellow-400">
                            <div class="flex">
                                <div class="flex-shrink-0">
                                ...
                                <div class="ml-3">
                                    ...
                                    <div class="mt-2 text-sm text-yellow-700">
                                        <p>Contact details (email, phone number) are hidden for privacy protection. To connect with this candidate, please request an interview using the button above.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
