    ...
    <nav class="fixed inset-x-0 top-0 z-40 h-16 border-b border-slate-200 bg-white/80 backdrop-blur">
        <div class="flex h-full items-center px-4 sm:px-6 lg:px-8 lg:pl-80 gap-4">
            ...
            <div class="flex items-center gap-3">
                <button class="hidden h-10 w-10 items-center justify-center rounded-full border border-slate-200 bg-white text-slate-600 shadow-sm hover:text-blue-600 focus:outline-none md:flex">
                    <i data-lucide="bell" class="h-4 w-4"></i>
                </button>
                <div x-data="{ open: false }" class="relative">
                    <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 rounded-full border border-slate-200 bg-white px-2 py-1 shadow-sm hover:border-blue-200">
                        <div class="h-9 w-9 overflow-hidden rounded-full border border-slate-200">
                            <img src="https://ui-avatars.com/api/?name={{ urlencode(auth()->user()->name) }}&background=2563eb&color=fff" alt="User" class="h-full w-full object-cover">
                        </div>
                    ...
                    <div x-show="open" x-cloak
                        ...
                        <div class="border-t border-slate-100 px-4 py-2">
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit" class="flex w-full items-center rounded-lg px-3 py-2 text-sm font-semibold text-red-600 hover:bg-red-50">
        ...
        <div class="flex h-full flex-col">
            ...
            <div class="flex-1 overflow-y-auto sidebar-scroll px-4 py-6 space-y-6">
                <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 shadow-lg shadow-blue-900/10">
                    <div class="flex items-center gap-3">
                        <div class="flex h-12 w-12 items-center justify-center rounded-full bg-blue-500/10 text-blue-300">
                ...
                <div>
                    ...
                    <nav class="space-y-1">
                        <x-admin-sidebar-item href="{{ route('admin.dashboard') }}" icon="layout-dashboard" label="Dashboard" :active="request()->routeIs('admin.dashboard')" />
                        <x-admin-sidebar-item href="{{ route('admin.candidates.index') }}" icon="users" label="Candidates" :active="request()->routeIs('admin.candidates.*')" />
                        <x-admin-sidebar-item href="{{ route('admin.verification.pending') }}" icon="shield-check" label="Verifications" :active="request()->routeIs('admin.verification.*')" />
                        <x-admin-sidebar-item href="{{ route('admin.categories.index') }}" icon="tag" label="Categories" :active="request()->routeIs('admin.categories.*')" />
                        <x-admin-sidebar-item href="{{ route('admin.jobs.index') }}" icon="briefcase" label="Jobs" :active="request()->routeIs('admin.jobs.*')" />
