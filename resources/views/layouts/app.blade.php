...
<head>
    ...
    <title>{{ config('app.name', 'COYZON') }} - @yield('title', 'Recruitment Platform')</title>
                        ...
                        <div x-data="{ open: false }" class="relative">
                            <button @click="open = !open" @click.away="open = false" class="flex items-center gap-3 rounded-full border border-slate-200 bg-white px-2 py-1 shadow-sm hover:border-blue-200">
                                <div class="h-9 w-9 overflow-hidden rounded-full border border-slate-200">
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode(optional(auth()->user())->name ?? 'User') }}&background=2563eb&color=fff" alt="User" class="h-full w-full object-cover">
                                </div>
                                <div class="hidden text-left text-sm font-semibold text-slate-800 md:block">
                                    <p class="leading-none">{{ auth()->user()?->name ?? 'User' }}</p>
                                    <span class="text-[11px] font-medium text-slate-500">@yield('page_label', 'Portal')</span>
                                </div>
                            ...
                            <div x-show="open" x-cloak
                                ...
                                <div class="border-b border-slate-100 px-4 py-3">
                                    ...
                                    <p class="text-sm font-semibold text-slate-900 truncate">{{ auth()->user()?->email ?? 'user@example.com' }}</p>
                                </div>
                                <div class="py-2">
                    ...
                    <div class="flex-1 overflow-y-auto sidebar-scroll px-4 py-6 space-y-6">
                        <div class="rounded-2xl border border-white/10 bg-white/5 px-4 py-4 shadow-lg shadow-blue-900/10">
                            <div class="flex items-center gap-3">
                                ...
                                <div class="min-w-0">
                                    <p class="text-sm font-semibold text-white truncate">{{ auth()->user()?->name ?? 'Welcome' }}</p>
                                    <p class="text-[12px] font-medium text-white/60 truncate">@yield('page_label', 'Portal') space</p>
                                </div>
                            </div>
                        </div>

                        <div>
                            <p class="px-2 text-[11px] font-semibold uppercase tracking-[2px] text-white/50 mb-3">Navigation</p>
                            <nav class="space-y-1">
                                @yield('sidebar')
                            </nav>
                        </div>
