...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-2xl font-bold text-gray-900">Candidate Requests</h2>
                <p class="text-sm text-gray-500">Manage candidate interview and placement requests from employers</p>
            </div>
        </div>
        ...
        <form method="GET" action="{{ route('admin.appointments.index') }}" class="flex flex-wrap gap-2">
            ...
            <button type="submit"
                class="px-6 py-2 bg-gray-200 text-gray-800 font-bold rounded-xl hover:bg-gray-300 transition-colors shadow-sm">
                Filter
            </button>
        </form>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                                Employer</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Type &
                                Schedule</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Payment
                            </th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($appointments as $appointment)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <div class="text-sm text-gray-900 font-bold">{{ ucfirst($appointment->appointment_type) }}
                                                </div>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @if($appointment->amount > 0)
                                                                    <div class="text-sm font-bold text-gray-900 mb-1">{{ number_format($appointment->amount) }}
                                                                        {{ $appointment->currency }}</div>
                                                                    <span
                                                                        class="px-2 py-0.5 inline-flex text-[10px] leading-4 font-bold rounded-full 
