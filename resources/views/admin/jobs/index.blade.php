    ...
    <div class="space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            ...
            <a href="{{ route('admin.jobs.create') }}"
                class="px-6 py-2 fb-blue-bg text-white font-bold rounded-xl hover:bg-blue-600 transition-colors shadow-sm flex items-center justify-center">
        ...
        <form method="GET" action="{{ route('admin.jobs.index') }}" class="flex flex-wrap gap-2">
            <div class="relative flex-1 min-w-[200px]">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by title or company..."
                    class="pl-10 pr-4 py-2 border border-gray-300 rounded-xl focus:ring-blue-500 focus:border-blue-500 w-full shadow-sm text-sm">
            ...
            <select name="employment_type"
                ...
                <option value="seasonal" {{ request('employment_type') == 'seasonal' ? 'selected' : '' }}>Seasonal</option>
            </select>
            ...
            @if(request()->anyFilled(['search', 'category_id', 'country', 'employment_type', 'status']))
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Job
                                Information</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">
                                Category & Type</th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Apps
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Status
                            </th>
                            <th class="px-6 py-3 text-left text-xs font-bold text-gray-500 uppercase tracking-wider">Posted
                            </th>
                            ...
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4">
                                    <div class="flex items-center">
                                        <div
                                            class="w-10 h-10 rounded-lg bg-blue-50 flex items-center justify-center mr-3 text-blue-600 font-bold border border-blue-100">
                                            {{ substr($job->company_name, 0, 1) }}
                                        </div>
                                        <div>
                                            <div class="text-sm font-bold text-gray-900">
                                                <a href="{{ route('admin.jobs.show', $job) }}"
                                                    class="hover:fb-blue">{{ $job->title }}</a>
                                            </div>
                                            <div class="text-xs text-gray-500">{{ $job->company_name }} • {{ $job->location }}
                                            </div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-2.5 py-0.5 rounded-full text-xs font-bold bg-gray-100 text-gray-800 border border-gray-200">
                                        {{ $job->applications->count() }}
                                    </span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span
                                        class="px-3 py-1 inline-flex text-xs leading-5 font-bold rounded-full {{ $job->is_active ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-600' }}">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-bold">
                                    <div class="flex justify-end space-x-3">
                                        ...
                                        <form method="POST" action="{{ route('admin.jobs.toggleStatus', $job) }}"
                                            class="inline">
                                            @csrf
                                            @method('PATCH')
