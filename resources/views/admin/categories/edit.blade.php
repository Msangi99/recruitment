...
<div class="max-w-2xl mx-auto space-y-6">
    <div class="flex items-center space-x-2 text-sm text-gray-500">
        <a href="{{ route('admin.categories.index') }}" class="hover:fb-blue flex items-center font-medium">
    ...
    <div class="bg-white shadow-sm border border-gray-200 rounded-xl overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-500">Updating category: <span class="font-bold fb-blue">{{ $category->name }}</span></p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="p-6">
            ...
            <div class="space-y-5">
                <div>
                    ...
                    @enderror
                    <p class="mt-2 text-xs text-gray-400">Current slug: <span class="font-mono">{{ $category->slug }}</span> (will be auto-updated)</p>
                </div>

                <div>
                    <label for="description" class="block text-sm font-bold text-gray-700 mb-2">Description</label>
                    <textarea id="description" name="description" rows="4" 
                              class="w-full border border-gray-300 rounded-xl px-4 py-2.5 focus:ring-blue-500 focus:border-blue-500 shadow-sm @error('description') border-red-300 @enderror">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="mt-2 text-sm text-red-600 font-medium">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-center p-4 bg-gray-50 rounded-xl border border-gray-100">
                    ...
                    <div class="ml-3 text-sm">
                        ...
                        <p class="text-gray-500">This category will be visible in job posting forms.</p>
                    </div>
                </div>
                ...
                @if($category->jobListings()->count() > 0)
                    <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 flex items-start">
                        ...
                        <p class="text-sm text-blue-700">
                            This category is used in <span class="font-bold">{{ $category->jobListings()->count() }}</span> job listing(s). 
                        </p>
                    </div>
                @endif
